 ...
 * 更新：全螢幕支援, 自定義 UI 菜單, 修正重新開始 Bug, 子彈速度再降
 */
...
// --- 狀態管理 ---
let gameState = {
    running: false, // 初始設為 false，等待點擊開始
    level: 1,
    exp: 0,
    nextLevelExp: 10,
    kills: 0,
...
class Boomerang extends Entity {
    ...
    reset() {
        this.returning = false; this.rotation = 0;
        this.returnThreshold = Math.max(canvas.width, canvas.height) / 2 + 50;
        const lvl = gameState.boomerangLevel;
        this.radius = 20 * (1 + (lvl - 1) * 0.5); this.damage = 1 + (lvl - 1) * 0.5;
    }
    update(px, py) {
        if (!this.active) return;
        if (!this.returning) {
            ...
            this.vx = (dx / d) * CONFIG.BOOMERANG_SPEED * 1.5; this.vy = (dy / d) * CONFIG.BOOMERANG_SPEED * 1.5;
            this.x += this.vx; this.y += this.vy;
    ...
    draw() {
        ...
        const sc = this.radius / 10;
        ctx.beginPath(); ctx.moveTo(-15*sc, -5*sc); ctx.lineTo(0, 15*sc); ctx.lineTo(15*sc, -5*sc); ctx.lineTo(0, 5*sc); ctx.closePath();
...
class XPGem extends Entity {
    ...
    update(px, py) {
        if (!this.active) return;
        const dx = px - this.x, dy = py - this.y, d = Math.sqrt(dx * dx + dy * dy);
        if (d < CONFIG.XP_ATTRACT_DIST) { this.x += (dx / d) * 7; this.y += (dy / d) * 7; }
    }
    draw() {
        const s = this.getScreenPos(); ctx.fillStyle = '#fbbf24';
        ctx.beginPath(); ctx.rect(s.x - 4, s.y - 4, 8, 8); ctx.fill();
    }
}
...
let activeEnemies = [], activeBullets = [], activeBoomerangs = [], activeXPGems = [];
...
function setupControls() {
    const move = (e) => {
        ...
        joystickData = { x: dx / max, y: dy / max };
    };
...
function spawnEnemy() {
    ...
    e.x = player.x + Math.cos(ang) * dist; e.y = player.y + Math.sin(ang) * dist;
...
function fireBullet() {
    ...
    if (closest) {
        ...
        if (b) {
            ...
            b.vx = (dx / d) * CONFIG.BULLET_SPEED; b.vy = (dy / d) * CONFIG.BULLET_SPEED;
...
function fireBoomerang() {
    ...
    if (target) {
        ...
        b.vx = (dx / d) * CONFIG.BOOMERANG_SPEED; b.vy = (dy / d) * CONFIG.BOOMERANG_SPEED;
    } else {
        const ang = Math.random() * Math.PI * 2;
        b.vx = Math.cos(ang) * CONFIG.BOOMERANG_SPEED; b.vy = Math.sin(ang) * CONFIG.BOOMERANG_SPEED;
    }
...
function checkCollisions() {
    activeBullets.forEach(b => {
        ...
        activeEnemies.forEach(e => {
            if (!e.active) return;
            if (Math.sqrt((b.x - e.x)**2 + (b.y - e.y)**2) < b.radius + e.radius) {
                b.active = false; if (e.takeDamage(b.damage)) { gameState.kills++; spawnXP(e.x, e.y); }
            }
        });
    });
    activeBoomerangs.forEach(b => {
        if (!b.active) return;
        activeEnemies.forEach(e => {
            if (e.active && Math.sqrt((b.x - e.x)**2 + (b.y - e.y)**2) < b.radius + e.radius) {
                if (e.takeDamage(b.damage)) { gameState.kills++; spawnXP(e.x, e.y); }
            }
        });
    });
...
function showUpgradeMenu() {
    ...
    const menu = document.getElementById('upgrade-menu'), opts = document.getElementById('upgrade-options');
    ...
    ].filter(i => i.id !== 'boomerang' || gameState.boomerangLevel < 4);
    ...
    selection.forEach(u => {
        ...
        div.onclick = () => { menu.classList.add('hidden'); gameState.upgradeMenuOpen = false; applyUpgrade(u.id); };
        opts.appendChild(div);
    });
}

function applyUpgrade(id) {
    if (id === 'speed') CONFIG.PLAYER_SPEED += 0.4;
    if (id === 'kunai') { CONFIG.FIRE_RATE *= 0.85; CONFIG.BULLET_SPEED += 0.8; }
    if (id === 'magnet') CONFIG.XP_ATTRACT_DIST += 50;
    if (id === 'boomerang') { gameState.hasBoomerang = true; gameState.boomerangLevel++; }
}
...
function updateUI() {
    ...
    document.getElementById('exp-bar-fill').style.width = `${(gameState.exp / gameState.nextLevelExp) * 100}%`;
    document.getElementById('hp-bar-fill').style.width = `${(player.hp / CONFIG.PLAYER_MAX_HP) * 100}%`;
...
function showGameOver() {
    ...
    document.getElementById('final-stats').innerText = `最終擊殺數: ${gameState.kills}`;
}

function resetGameState() {
    gameState.level = 1; gameState.exp = 0; gameState.nextLevelExp = 10;
    gameState.kills = 0; gameState.time = 0; gameState.upgradeMenuOpen = false;
    gameState.currentSpawnChance = CONFIG.INITIAL_SPAWN_CHANCE;
    gameState.hasBoomerang = false; gameState.boomerangLevel = 0;
    player.reset();
    enemyPool.releaseAll(); bulletPool.releaseAll(); boomerangPool.releaseAll(); xpPool.releaseAll();
    activeEnemies = []; activeBullets = []; activeBoomerangs = []; activeXPGems = [];
    updateUI();
}
